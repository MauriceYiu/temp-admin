...
var LinkRoom = function (ctr, data) {
  ...
  var doLink = function (data) {
        ...
        that.room.getChatroomMembersInfo({
          accounts: [that.room.account],
            ...
            if (
              ...
              obj.members &&
              obj.members[0] &&
              obj.members[0].avatar
            ) {
              var avatar = obj.members[0].avatar;
              util.setCookie('avatar', avatar);
              document.getElementById('avatar').src = util.getAvatar(
                util.readCookie('avatar')
              );
            }
          }
        });
          ...
          switch (error.code) {
            ...
            case 'kicked':
              if (error.reason === 'managerKick') {
                util.setCookie(
                  'kickReason',
                  '你已被管理员移出|' + that.data.roomInfo.name
                );
              } else if (error.reason === 'blacked') {
                util.setCookie(
                  'kickReason',
                  '你已被管理员拉入黑名单，不能再进入|' +
                  that.data.roomInfo.name
                );
              }
    ...
    if (!data.token) {
      ...
      data.chatroomNick = options.chatroomNick =
        '游客' + Math.ceil(Math.random() * 10000);
  ...
  if (typeof CONFIG.chatroomAddr === 'object' && Object.prototype.toString.call(CONFIG.chatroomAddr) === '[object Array]') {
    ...
    $.ajax({
      url: window.chatroomAddr,
      contentType: 'application/json',
      type: 'POST',
      beforeSend: function (req) {
        req.setRequestHeader('appkey', CONFIG.appkey);
        req.setRequestHeader('User-Token', userToken);
      },
      data: JSON.stringify({
        roomId: info.id,
        userId: info.account,
      ...
      let resData = data;
      if (window.isAes) {
        // resData = decryptStr(JSON.parse(data))
        resData = decryptStr(data)
      }
      if (resData.status === 200) {
        getAddr(resData.data);
      } else {
        alert('获取连接房间地址失败');
      }
    });
