...
var page = {
    init: function() {
        var options = {
            ...
            chatroomNick: localStorage.getItem("name"),
    ...
    initEvt: function() {
        ...
        this.$clearText.on("click", ()=> {
          this.link.sendText("*(0-q+p-0)*", (err, data)=>{
            alert('客户端已清屏')
          })
        });
    ...
    buildMsgUI: function(item) {
        ...
        var vip = item.fromCustom ? ("<span  class=\"text sys\">(" + JSON.parse(item.fromCustom).vip + ") </span>") : "";
        return ['<div class="item" data-id="' + item.idClient + '">',
            '<p class="nick ' + type + ' j-nick ' + roleClass + '" data-role="' + type + '" data-account="' + item.from + '">' + ((type === "owner" || type === "manager") ? '<b class="role role-' + type + '"></b>' : "") + vip + item.fromNick + '</p>',
    ...
    buildSysMsgUI: function(item) {
        var vip = item.fromCustom ? ("(" + JSON.parse(item.fromCustom).vip + ") ") : "";
        return ['<div class="item" data-id="' + item.idClient + '">',
            '<p class="text sys">' + vip + util.buildSysMsg(item) + '</p>',
            '</div>'
        ].join("");
    },
    //成员列表UI绘制
