    ...
    switch (data.type) {
      ...
      case 'custom':
        if (!data.content) {
          ...
          console.log(content);
          if (content.type === 100 || content.msgType === 100) {
            let data = JSON.parse(content.data);
            text = "赠送礼物 [" + data.name + "] x " + data.gift_count;
          } else if (content.type === 101 || content.msgType === 101) {
            text = "爱心x1";
          } else if (content.type === 102 || content.msgType === 102) {
            text = "巴巴掌x1";
          } else {
            text = data.content;
          }
