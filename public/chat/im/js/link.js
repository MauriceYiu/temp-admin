...
SDKBridge.prototype.sendFileMessage = function (scene, to, fileInput, callback) {
  var that = this,
    ...
    type = /png|jpg|bmp|jpeg|gif/i.test(ext) ? 'image' : (/avi|mov|rmvb|rm|flv|mp4|3gp/i.test(ext) ? 'video' : 'file');
  this.nim.sendFile({
    scene: scene,
    to: to,
    type: type,
    ...
    uploadprogress: function (data) {
      $("#fileProgress").text(data.percentageText);
    ...
    beforesend: function (msg) {
      console.log(msg, "正在发送消息")
    },
    done: callback
  });
};
SDKBridge.prototype.sendBlobFile = function (scene, to, name, dataURL, callback) {
  var that = this,
    value = name,
    ext = value.substring(value.lastIndexOf('.') + 1, value.length),
    type = /png|jpg|bmp|jpeg|gif/i.test(ext) ? 'image' : (/avi|mov|rmvb|rm|flv|mp4|3gp/i.test(ext) ? 'video' : 'file');
 ...
 * 群成员静音
 */
SDKBridge.prototype.updateMuteStateInTeam = function (id,
                                                      account,
                                                      mute,
                                                      callback) {
...
SDKBridge.prototype.updateMyInfo = function (nick,
                                             gender,
                                             birth,
                                             tel,
                                             email,
