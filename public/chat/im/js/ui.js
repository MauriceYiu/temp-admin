var appUI = {
  ...
  buildChatContentUI: function (id, cache) {
    console.log(id, cache, "idididididid");
    var msgHtml = '',
  ...
  makeChatContent: function (message, user) {
    //console.log(message, user, "makeChatContent");
    let id = user.account;
    var msgHtml;
    //通知类消息
    if (
      ...
      var type = message.type,
        from = message.from,
        // avatar = user.avatar,
        avatar = window.API_BASE_URI + "/avatar/" + id,
      ...
      if (type === 'tip') {
        ...
        msgHtml = [
          ...
          '<div class="msg msg-text j-msg msg-click" data-msgcont='+JSON.stringify(message)+' data-msgtype='+ message.type +'>',
          '<div class="box">',
          '<div class="cnt">',
          getMessage(message),
          '</div>',
