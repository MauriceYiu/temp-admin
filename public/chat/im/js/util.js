...
function buildSessionMsg(msg) {
    //console.log(msg, "buildSessionMsg");
...
function getMessage(msg) {
    //console.log(msg, "getMessage", msg.type);
    var str = ''
    ...
    switch (msg.type) {
        ...
        case 'image':
            if (msg.status === -1) {
                ...
                str = '<a href="javaScript:;"><img onload="loadImg()" data-magnify="gallery" data-src="' + url + '" src="' + url + '?imageView&thumbnail=200x0&quality=85"/></a>';
            }
            break;
        case 'file':
            if (msg.status === -1) {
            ...
            if (msg.custom) {
                ...
                switch (parseInt(custom.type)) {
                    ...
                    case 110:
                        ...
                        break;
                    case 111:
                        str = '分享了[' + custom.title + ']';
                        break;
                    default:
                        str = sentStr + '未知提示' + custom.type;
                        break;
                }
            } else {
                str = msg.tip;
        ...
        case 'custom':
            if (msg.content) {
                ...
                switch (parseInt(content.type)) {
                    case 100:
                        if (content.content) {
                            str += "<div class='f-maxWid'>" + content.content + "</div>"
                        }
                        if (content.image) {
                            str += '<a href="' + content.image + '" target="_blank"><img onload="loadImg()" data-src="' + content.image + '" src="' + content.image + '?imageView&thumbnail=200x0&quality=85"/></a>';
                        }
                        break;
                    case 101:
                    case 102:
                    ...
                    case 110:
                        ...
                        break;
                    case 111:
                        str += "<div class='customShare'>"
                        str += '<img onload="loadImg()" data-src="' + content.image + '" src="' + content.image + '?imageView&thumbnail=200x0&quality=85"/>';
                        str += "<div class='customContent'>";
                        str += "<div class='customTitle'>" + content.title + "</div>"
                        str += '<div class="customType">' + redirectTypeText(content.redirectType) + '</div>';
                        str += "</div></div>";
                        break;
                    case 112:
                        str = '退币成功';
...
function redirectTypeText(type) {
    switch (type) {
        case 'none':
            return '无';
        case 'url':
...
function getIdAvatar(id) {
    return window.API_BASE_URI + "/avatar/" + id;
}
