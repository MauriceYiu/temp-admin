    ...
    methods: {
      ...
      tagSubmitForm() {
        ...
        pkWin(this.pkWinParams).then(res => {
          if (res.status) {
            this.$message.success("PK结束，到已判定列表可查看！");
...
<style>
  ...
  .PKlistPersonal .filter-container .left .el-tabs__active-bar {
    background: none;
  }

  .PKlistPersonal .el-tabs__content {
    display: none;
  }

  .PKlistPersonal .filter-container .filter-item {
    margin-bottom: 0;
  }

  .PKlistPersonal .el-tabs__nav-wrap::after {
    background: none;
  }

  .PKlistPersonal .filter-container .left .el-tabs__nav {
    margin-left: 0;
  }
</style>
