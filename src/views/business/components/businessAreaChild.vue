<template>
  <div class="businessProductChild">
    ...
    <div class="filter-container">
      <div class="left">
        <span :class="nowFilterType=='0'?'active':''" @click.stop="nowFilterType='0'">全部</span>
        <span :class="nowFilterType=='1'?'active':''" @click.stop="nowFilterType='1'">启用</span>
        <span :class="nowFilterType=='2'?'active':''" @click.stop="nowFilterType='2'">禁用</span>
      </div>
      <!--<div class="right">-->
    ...
    <el-table
      ...
      <el-table-column align="center" label="内容">
        <template slot-scope="scope">
          <span class="intro">{{ scope.row.detail}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="教师数量">
        <template slot-scope="scope">
          ...
          <div class="action">
            <div class="status">
              ...
              <el-switch
                ...
                style="display: block"
                v-if="hasPerm('business:serviceArea:enable')"
                v-model="scope.row.enable"
                active-color="#13ce66">
              </el-switch>
            </div>
            <el-button type="text" size="mini" @click="delTag(scope.row)" style="color: #DADADA" v-if="hasPerm('business:serviceArea:delete')">删除</el-button>
            <el-button type="text" size="mini" @click="addTag(scope.row.id)" style="color: #F65860" v-if="hasPerm('business:serviceArea:add')">编辑</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      changeStatus(value, item) {
        ...
        areaEnable(this.enableParams).then(json => {
          if (json.status === 200) {
            console.log(json);
      ...
      delTag(data) {
        ...
        this.$confirm("此操作将删除该业务领域, 是否继续?", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        })
          .then(() => {
            areaDel(this.serviceDelParams).then(res => {
              // console.log(res,"res");
              if (res.status) {
