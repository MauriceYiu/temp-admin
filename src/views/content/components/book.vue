    ...
    <div class="filter-container">
      <div class="left">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          ...
          <el-tab-pane label="优惠" name="second"></el-tab-pane>
          <el-tab-pane label="免费" name="third"></el-tab-pane>
        </el-tabs>
      </div>
      <div class="right">
    ...
    <el-table
      ...
      <el-table-column align="center" label="价格/优惠价">
        <template slot-scope="scope">
          ...
          <span v-else>{{scope.row.price | showDecimal}} / {{ getPreferPriceFormat(scope.row) }} </span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="收入">
        <template slot-scope="scope">
          <span>{{scope.row.saleMoney | showDecimal}}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <div class="action">
            <div class="status" v-if="hasPerm('content:course:enable')">
...
// 收藏类型：book书本课程、news资讯、tech技术课程收藏类型：book书本课程、news资讯、tech技术课程
    ...
    methods: {
      getPreferPriceFormat(data){
        if(data.preferPrice == -1){
          return  '无优惠'
        }else {
          return showDecimal(data.preferPrice)
        }
      },
...
<style>
  ...
  .book .filter-container .left .el-tabs__active-bar {
    background: none;
  }

  .book .el-tabs__content {
    display: none;
  }

  .book .filter-container .filter-item {
    margin-bottom: 0;
  }

  .book .filter-container .left .el-tabs__nav {
    margin-left: 0;
  }
</style>
