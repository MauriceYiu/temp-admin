  ...
  <div class="liveWatch">
    ...
    <el-table
      ...
      <el-table-column align="center" label="名称">
        <template slot-scope="scope">
          <span>{{scope.row.live.title}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="直播封面">
        <template slot-scope="scope">
          <img style="width: 50px" :src="scope.row.live.coverImage" alt="">
        </template>
      </el-table-column>
      <el-table-column align="center" label="价格">
        <template slot-scope="scope">
          <span>{{scope.row.live.price | showDecimal}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="时间">
        <template slot-scope="scope">
          <span>{{scope.row.live.createTime | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
        </template>
      </el-table-column>
...
<style>
  ...
  .liveWatch .filter-container .left .el-tabs__active-bar {
    background: none;
  }

  .liveWatch .el-tabs__content {
    display: none;
  }

  .liveWatch .filter-container .filter-item {
    margin-bottom: 0;
  }
</style>
