<template>
  <div id="giftLive">
    ...
    <el-table :data="list" row-key="id"  style="width: 100%;">
      ...
      >
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="delTag(scope.row)" style="color: #DADADA" v-if="hasPerm('settings:gifts:live:delete')">删除</el-button>
          <el-button type="text" size="mini" @click="add(scope.row.id)" style="color: #F65860" v-if="hasPerm('settings:gifts:live:edit')">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      delTag(data) {
        ...
        this.$confirm("此操作将删除该直播礼物, 是否继续?", "提示", {
      ...
      add(data){
        this.$router.push({ name: 'giftLiveDetail',params:{
            id:data
          }})
      },
