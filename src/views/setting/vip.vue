<template>
  <div class="vip">
    ...
    <el-table :data="list" row-key="id" border lazy :load="load" style="width: 100%;">
      ...
      <!--<template slot-scope="scope">-->
      <!--<div class="imbox">-->
      <!--<div class="imgWrap" v-for="(img,index) in scope.row.images" :key="index">-->
      ...
      <el-table-column align="center" label="课程所有人都可购买">
        <template slot-scope="scope">
          <span>{{scope.row.anyoneBuy?"是":"否"}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="课程高于此级别的免费">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="累计充值悟空币">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="addVip(scope.row.id)">编辑</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addVip(data){
        this.$router.push({ name: 'vipDetail',params:{
            id:data
          }})
      },
      addTag(data) {
