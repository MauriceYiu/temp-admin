  ...
  <div class="kefu">
    ...
    <el-table
      ...
      <el-table-column align="center" label="性别">
        <template slot-scope="scope">
          <span v-if="scope.row.userGender">{{scope.row.userGender.text}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="服务结束时间">
        <template slot-scope="scope">
          <span>{{scope.row.createTime | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="咨询类型">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          <div class="action">
            ...
            <el-button type="text" size="mini" @click="addTag(scope.row)" style="color: #F65860" v-if="hasPerm('customers:afterSale:customerService:detail')">查看</el-button>
...
<script>
  import {chatCsEvaluates, userEnable} from "../../../api/user";
...
<style>
  ...
  .kefu .filter-container .left .el-tabs__active-bar {
    background: none;
  }

  .kefu .el-tabs__content {
    display: none;
  }

  .kefu .filter-container .filter-item {
    margin-bottom: 0;
  }
</style>
